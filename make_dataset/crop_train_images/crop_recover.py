...
def test_and_complement(image, crop_height, crop_width):
	if image.shape[0] != crop_height or image.shape[1] != crop_width:
		complement = np.zeros([crop_height, crop_width, image.shape[2]]).astype(np.uint8)
...
def crop_image(image, crop_height, crop_width, oc):
	...
	output = np.zeros([total_output_number, crop_height, crop_width, image.shape[2]]).astype(np.uint8)
	count = 0
...
def recover_image(cropped_image, height, width, crop_height, crop_width, oc):
	...
	output_image = np.zeros([in_height_number*crop_height, in_width_number*crop_width, cropped_image.shape[3]]).astype(np.uint8)
